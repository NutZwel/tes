...
function switchTab(id){
  ...
  var el=document.getElementById(id); if(el) el.classList.add('profile-panel--active');
  var btn=document.querySelector('[data-tab="'+id+'"]'); if(btn) btn.classList.add('profile-tabs__btn--active');
  // Update hash so refresh stays on tab
...
/* ═══ Instant theme save (no page reload) ═══ */
function saveTheme(key, val) {
  var x = new XMLHttpRequest();
  x.open('POST', BASE+'user/save_pref_ajax', true);
  x.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
  x.onload = function() {
    if (x.status === 200) {
      // Reload page to apply theme to entire layout
      window.location.reload();
    }
  };
  x.send('field='+encodeURIComponent(key)+'&value='+encodeURIComponent(val));
}
function saveThemeStay() {
  var x = new XMLHttpRequest();
  x.open('POST', BASE+'user/save_pref_ajax', true);
  x.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
  x.onload = function() {
    if (x.status === 200) {
      // Stay on preferences tab
